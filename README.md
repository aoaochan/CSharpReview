# CSharpReview

## Journal

### Day 1 (2026/06/28/Sunday)
- [x] **Start Of Project**
- [ ] Learning - [C#을 사용하여 첫 번째 코드 작성(C#시작, 1부)]: [🔗](https://learn.microsoft.com/en-us/training/paths/get-started-c-sharp-part-1/)
- [x] Tutorial - 첫 번째 C# 코드 작성 (Tutorial1.cs): [🔗](https://learn.microsoft.com/en-us/training/modules/csharp-write-first/)
- [ ] Tutorial - C에서 리터럴 및 변수 값을 사용하여 데이터 저장 및 검색 (Tutorial2.cs): [🔗](https://learn.microsoft.com/en-us/training/modules/csharp-literals-variables/)

### Day 2 (2026/06/29/Monday)
- [ ] Learning - [C#을 사용하여 첫 번째 코드 작성(C#시작, 1부)]: [🔗](https://learn.microsoft.com/en-us/training/paths/get-started-c-sharp-part-1/)
- [x] Tutorial - C에서 리터럴 및 변수 값을 사용하여 데이터 저장 및 검색 (Tutorial2.cs): [🔗](https://learn.microsoft.com/en-us/training/modules/csharp-literals-variables/)
- [ ] Tutorial - Perform basic string formatting in C# (Tutorial3.cs): [🔗](https://learn.microsoft.com/en-us/training/modules/csharp-basic-formatting/)

### Day 3 (2026/07/01/Wednesday)
- [ ] Learning - [C#을 사용하여 첫 번째 코드 작성(C#시작, 1부)]: [🔗](https://learn.microsoft.com/en-us/training/paths/get-started-c-sharp-part-1/)
- [x] Tutorial - Perform basic string formatting in C# (Tutorial3.cs): [🔗](https://learn.microsoft.com/en-us/training/modules/csharp-basic-formatting/)
- [x] Tutorial - Perform basic operations on numbers in C# (Tutorial4.cs): [🔗](https://learn.microsoft.com/en-us/training/modules/csharp-basic-operations/)
- [x] Tutorial - Guided project - Calculate and print student grades (Tutorial5.cs) [🔗](https://learn.microsoft.com/en-us/training/modules/guided-project-calculate-print-student-grades/)